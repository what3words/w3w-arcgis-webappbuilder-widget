    ...
    */
    build: function(options){
      ...
      if(this.isBuilding()){
        ...
        this.useStandardizedQueries = this._filterUtils.useStandardizedQueries(options.layerDefinition)
        if(options.partsObj){
    ...
    // Handle `useStandardizedQueries` prop for Set, Single experience
    _handleUseStandardizedQueries: function(part) {
      if (part && this.useStandardizedQueries) {
        if (part.parts) {
          array.forEach(part.parts, lang.hitch(this, function(singlePart){
            if (singlePart.fieldObj.shortType === 'date') {
              singlePart._useStandardizedQueries = this.useStandardizedQueries
            }
          }));
        } else if(part.fieldObj.shortType === 'date') {
          part._useStandardizedQueries = this.useStandardizedQueries
        }
      }
      return part
    },
    ...
    toJson:function(){
      ...
      array.forEach(filters, lang.hitch(this, function(filter){
        ...
        part = this._handleUseStandardizedQueries(part)
