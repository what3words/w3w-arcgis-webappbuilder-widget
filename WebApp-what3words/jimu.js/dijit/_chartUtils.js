...
define([
  ...
  var ChartUtils = declare([], {
    ...
    _getValidRange: function(value, type, defaultMin, defaultMax) {
      var valid = false
      valid = value != null && (value >= defaultMin && value <= defaultMax)
      var defaultValue = type === 'max' ? defaultMax : defaultMin
    ...
    settingStack: function(option, config) {
      ...
      if (config.stack === 'percent') {
        ...
        if (config.type === 'bar') {
          option.xAxis.max = this._getValidRange(option.xAxis.max, 'max', defaultMin, defaultMax);
          option.xAxis.min = this._getValidRange(option.xAxis.min, 'min', defaultMin, defaultMax);
        } else {
          option.yAxis.max = this._getValidRange(option.yAxis.max, 'max', defaultMin, defaultMax);
          option.yAxis.min = this._getValidRange(option.yAxis.min, 'min', defaultMin, defaultMax);
        }
      }
